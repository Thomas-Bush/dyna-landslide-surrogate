...
class FinalProcessor:
    def __init__(self, root_directory, model_id, target_size, target_resolution, interpolation_order):
        """Initializes a FinalProcessor instance with specified attributes.

        Args:
            root_directory (str):       The root directory where model data is stored.
            model_id (int):             The unique identifier for the model.
            target_size (int):          The desired dimension to which arrays will be resized, assuming square arrays.
            target_resolution (float):  The spatial resolution of each array cell, assuming square cells.
            interpolation_order (int):  The order of the interpolation used in resizing. Higher orders provide better
                                        accuracy but are computationally more expensive.
        """
