...
class FinalProcessor:
    ...
    def crop_or_pad_array_to_fixed_size(self, array, target_size=512, bbox=None):
        """Crop or pad the input array to a target size to cover the bounding box of the first thickness state array.
        ...
        Args:
            ...
            bbox (dict): The bounding box of the first thickness state array with keys 'min_x', 'max_x', 'min_y', 'max_y'.
        ...
        # Ensure the cropped/padded array covers the bounding box of the first thickness state array
        crop_x1 = max(0, min_x)
        crop_x2 = min(current_width, max_x + 1)
        crop_y1 = max(0, min_y)
        crop_y2 = min(current_height, max_y + 1)

        # Calculate the position where the cropped array will be placed in the new_array
        # The cropped area should start at (0,0) in the new_array
        paste_x1 = 0
        paste_y1 = 0
        paste_x2 = crop_x2 - crop_x1
        paste_y2 = crop_y2 - crop_y1
        ...
        # If the target size is larger than the bounding box, we pad the rest
        if paste_x2 < target_size:
            pad_size_right = target_size - paste_x2
            new_array[:, paste_x2:] = 0  # pad with zeros (or other value if needed)
        
        if paste_y2 < target_size:
            pad_size_bottom = target_size - paste_y2
            new_array[paste_y2:, :] = 0  # pad with zeros (or other value if needed)

        return new_array

         
    def crop_or_pad_arrays(self, target_size=512):
        """Crops or pads all arrays to a target size based on the bounding box of the first thickness state."""
        self.calculate_bounding_boxes()
        
        first_thickness_bbox = self.data['metadata_dict']['bounding_boxes'].get('2')  # 2 is the first state with any data

        

        if first_thickness_bbox is None:
            raise ValueError("Bounding box for the first thickness state is not available.")

        # Crop or pad elevation array
        elevation_array = self.data['elevation_array_dict']['z_values']
        self.data['elevation_array_dict']['z_values'] = self.crop_or_pad_array_to_fixed_size(
            elevation_array, target_size, first_thickness_bbox)
        ...
        for key, thickness_array in self.data['thickness_state_array_dict'].items():
            self.data['thickness_state_array_dict'][key] = self.crop_or_pad_array_to_fixed_size(
                thickness_array, target_size, first_thickness_bbox)
        ...
        for key, velocity_array in self.data['velocity_state_array_dict'].items():
            self.data['velocity_state_array_dict'][key] = self.crop_or_pad_array_to_fixed_size(
                velocity_array, target_size, first_thickness_bbox)


    def create_mask(self):
        """Create a mask array indicating the padded area."""
        elevation_array = self.data['elevation_array_dict']['z_values']
        mask = np.ones_like(elevation_array, dtype=np.float32)

        # Find the indices where the elevation array is zero (padded area)
        padded_indices = np.where(elevation_array == 0)

        # Set the corresponding elements in the mask to zero
        mask[padded_indices] = 0.0
    ...
    def export_data(self, base_path):
        ...
        rounding all values to 2 decimal places. Also create a "gan" folder with "input"
        and "target" subfolders containing the specified data.
        ...
        # Create the "gan" folder with "input" and "target" subfolders
        ...
        mask_array = self.create_mask()

        # Copy the elevation data and the first thickness array to the "input" folder
        elevation_input_path = os.path.join(input_path, f'{self.model_id}_elevation.npy')
        thickness_input_path = os.path.join(input_path, f'{self.model_id}_thickness_0.npy')
        np.save(elevation_input_path, elevation_array)
        np.save(thickness_input_path, list(self.data['thickness_state_array_dict'].values())[0])

        # Save the mask array in the "input" folder
        mask_input_path = os.path.join(input_path, f'{self.model_id}_mask.npy')
        np.save(mask_input_path, mask_array)

        # Stack the max values from all velocity and thickness arrays and save them in the "target" folder
        max_velocity_array = np.max(np.stack(list(self.data['velocity_state_array_dict'].values())), axis=0)
        max_thickness_array = np.max(np.stack(list(self.data['thickness_state_array_dict'].values())), axis=0)

        velocity_target_path = os.path.join(target_path, f'{self.model_id}_velocity_max.npy')
        thickness_target_path = os.path.join(target_path, f'{self.model_id}_thickness_max.npy')

        np.save(velocity_target_path, max_velocity_array)
        np.save(thickness_target_path, max_thickness_array)
