...
class DebrisStatePairsDataset(Dataset):
    ...
    def scale_data(self, terrain, velocity, thickness):
        ...
        terrain_scaled = (terrain - min_elevation) / (max_elevation - min_elevation) * 10
...
#         # Compute min and max values over the subset
#         for idx in subset.indices:
#             current_velocity_path, _, current_thickness_path, _ = self.data_files[idx]
...
#             velocity = np.load(current_velocity_path)
#             thickness = np.load(current_thickness_path)
...
class DebrisStateSeriesDataset(Dataset):
    def __init__(self, root_dir, array_size, sequence_length, apply_scaling=False):
        self.array_size = array_size
        self.data_files = []
        self.model_ids = []  # List to store the model IDs
        self.terrain_files = {}
